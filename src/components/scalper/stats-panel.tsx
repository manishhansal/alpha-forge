...
import type { JournalStats, StrategyStats, SymbolStats } from "@/features/scalping/journal";
...
export function StatsPanel({ stats }: { stats: JournalStats }) {
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-[12px] text-[var(--color-fg-muted)]">
            No paper trades have fired yet. Start the worker (
            <code className="font-mono">npm run worker:dev</code>) and the scalper will begin
            opening trades automatically as new UT Bot signals confirm.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat label="Total" value={overall.total.toString()} hint={`${overall.open} open`} />
          <Stat label="Win rate" value={pct(overall.winRate)} hint={`${overall.wins}W / ${overall.losses}L`} />
          <Stat
            label="Net P&L"
            value={`${pnlText(overall.totalPnlUsd, 2)}`}
            valueClass={pnlClass(overall.totalPnlUsd)}
            hint={`per ${overall.total > 0 ? "$1k" : "—"}`}
          />
          <Stat
            label="Profit factor"
                ...
                {byStrategy.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-3 py-3 text-[var(--color-fg-subtle)]">
                      No closed trades yet across the active strategies.
                    </td>
                  </tr>
                ) : (
                  byStrategy.map((s) => <StrategyRow key={s.strategyId} stats={s} />)
                )}
              </tbody>
            </table>
          </div>
...
function StrategyRow({ stats }: { stats: StrategyStats }) {
  const meta = getStrategyMeta(stats.strategyId);
  return (
    <tr className="border-t border-[var(--color-border)]">
      ...
      <Td align="right">{stats.wins} / {stats.losses}</Td>
...
function SymbolRow({ stats }: { stats: SymbolStats }) {
  return (
    <tr className="border-t border-[var(--color-border)]">
      <Td>
