...
const SYMBOLS: ReadonlyArray<"ALL" | SymbolId> = ["ALL", "BTC", "ETH", "SOL"];
const STATUSES: ReadonlyArray<"ALL" | PaperTradeStatus> = [
  "ALL",
  "OPEN",
  "WIN",
  "LOSS",
...
export function JournalCard() {
  ...
  const { selected, pairs } = useStrategyFilter();
  ...
  // Server already paginated + filtered the rows — render exactly what
  // came back. Index range is derived from the page math, not the array
  // length, so the "Showing X–Y of Z" label stays accurate even if the
  // current page legitimately came back short (e.g. last page).
      ...
      <CardHeader className="flex flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <FilterSelect
            label="Symbol"
            value={symbol}
            onChange={(v) => setSymbol(v as "ALL" | SymbolId)}
            options={[...SYMBOLS]}
          />
          <FilterSelect
            label="Status"
            value={status}
            onChange={(v) => setStatus(v as "ALL" | PaperTradeStatus)}
            options={[...STATUSES]}
          />
        </div>
      </CardHeader>
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {items.map((t) => {
                  ...
                  // For OPEN rows mark-to-market; for closed rows the
                  // server already persisted the realised values.
                  const live = isOpen ? computeLivePnl(t, prices[t.symbol]) : null;
                  ...
                  const exitDisplay = isOpen
                    ? prices[t.symbol] !== undefined
                      ? `$${formatPrice(prices[t.symbol] as number)}`
                      : "—"
                    : t.exitPrice !== null
                  ...
                  return (
                    ...
                    >
                      ...
                      <Td align="right" className={pnlClass(pnlPct)}>
                        {pnlPct !== null
                          ? `${pnlPct > 0 ? "+" : ""}${pnlPct.toFixed(2)}%`
                          : "—"}
                      </Td>
                      <Td align="right" className={pnlClass(pnlUsd)}>
                        {pnlUsd !== null
                      ...
                      <Td>
                        {editing === t.id ? (
                          ...
                          >
                            {t.note ?? <span className="text-[var(--color-fg-subtle)]">add…</span>}
                          </button>
                        )}
                      </Td>
                    </tr>
...
/**
 * Compact pagination strip rendered under the journal table. Always
 * visible when at least one row is shown so the user understands the
 * "10 per page" cap (otherwise the missing rows feel like a bug).
 *
 * Page numbers are rendered as chips capped at 7 visible slots — long
 * runs are condensed with an ellipsis (e.g. `1 … 4 5 6 … 12`) so the
 * footer never wraps onto multiple lines on a wide journal.
 */
function PaginationFooter({
  ...
  pageEnd: number;
  total: number;
  /** Lock the controls while a fetch is in flight to prevent rapid
   *  double-clicks from racing the request id check. */
  disabled?: boolean;
  onPrev: () => void;
  onNext: () => void;
...
/**
 * Build the chip list for the pagination strip:
 *   - Always shows the first and last page.
 *   - Always shows the current page and its neighbours.
 *   - Bridges gaps with an ellipsis so the strip stays compact even when
 *     the user is on page 47 of 120.
 */
function buildPageList(current: number, total: number): Array<number | "…"> {
  ...
  // Clamp + sort the unique pages we want to render.
  const pages = [...set]
